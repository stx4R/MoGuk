...
-- RLS 우회 없이 글로벌방 여부 확인 — chat_messages INSERT 정책에서 RLS 간섭 차단 S
create or replace function public.is_global_room(p_room_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
as $$
  select exists (
    ...
    where id = p_room_id and is_global = true
  );
$$;

-- 사용자가 해당 방에 자가 가입 가능한지 확인 (공개방·글로벌방·정당방만 허용) S
create or replace function public.can_self_join_room(p_room_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
as $$
  select exists (
    ...
    where cr.id = p_room_id
      and (
        cr.is_public  = true
        or cr.is_global = true
        or (cr.party_tag is not null
            and cr.party_tag = (select pp from public.profiles where id = auth.uid()))
      )
  );
$$;
...
-- 일반 유저는 프로필을 직접 수정할 수 없음 (admin-only 필드 조작 공격 방어) S
-- Admin은 RLS로, 일반 Admin RPC(admin_ban_user 등)는 SECURITY DEFINER로 업데이트 수행
create policy "Admin만 프로필 수정 가능"
...
-- 전체 공지방 admin 전용 체크는 is_global_room() SECURITY DEFINER로 RLS 간섭 차단 S
create policy "방 멤버이고 미차단 상태면 메시지 전송 가능"
  ...
  with check (
    ...
    and (not public.is_global_room(chat_messages.room_id) or public.is_admin())
  );
...
-- 자가 가입: 공개방·글로벌방·정당방만 허용 / 타인 초대: Admin 또는 방 개설자만 가능 S
create policy "채팅방 멤버 가입 규칙"
  on public.chat_room_members for insert to authenticated
  with check (
    not public.is_restricted()
    and (
      -- 본인이 공개·글로벌·정당방에 자가 가입
      (auth.uid() = user_id and public.can_self_join_room(room_id))
      or
      -- Admin 또는 방 개설자가 타인을 초대
      (
        auth.uid() != user_id
        and (
          public.is_admin()
...
-- -----------------------------------------------------------------
-- 7-B. 보안 트리거 — 프로필 조작 공격 방어
-- -----------------------------------------------------------------

-- profiles.name 영구 불변 + 비관리자의 admin-only 필드 변조 차단 S
...
-- auth.users.user_metadata.name 변조 시 profiles.name으로 강제 복원 S
-- supabase.auth.updateUser({ data: { name: ... } })로 이름 조작하는 것을 차단
create or replace function public.prevent_user_metadata_tampering()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  v_profile_name text;
begin
  if old.raw_user_meta_data->>'name' is distinct from new.raw_user_meta_data->>'name' then
    select name into v_profile_name from public.profiles where id = new.id;
    if found and v_profile_name is not null then
      new.raw_user_meta_data := jsonb_set(
        coalesce(new.raw_user_meta_data, '{}'::jsonb),
        '{name}',
        to_jsonb(v_profile_name)
      );
    end if;
  end if;
  return new;
...
create trigger prevent_user_metadata_tampering
before update on auth.users
for each row execute function public.prevent_user_metadata_tampering();
