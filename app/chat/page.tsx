...
export default function ChatPage() {
  return (
    <div className="min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center px-6 text-center">
      <style>{`
        @keyframes spin-disc-ch { to { transform: rotate(360deg); } }
        @keyframes needle-in-ch { from { transform: rotate(-20deg); } to { transform: rotate(25deg); } }
        @keyframes fade-up-ch { from { opacity: 0; transform: translateY(18px); } to { opacity: 1; transform: translateY(0); } }
        .disc-ch { animation: spin-disc-ch 4s linear infinite; }
        .needle-ch { animation: needle-in-ch 0.9s cubic-bezier(0.34,1.56,0.64,1) 0.3s both; transform-origin: top right; }
        .content-ch > * { animation: fade-up-ch 0.6s ease both; }
        .content-ch > *:nth-child(1) { animation-delay: 0.1s; }
        .content-ch > *:nth-child(2) { animation-delay: 0.22s; }
        .content-ch > *:nth-child(3) { animation-delay: 0.34s; }
        .content-ch > *:nth-child(4) { animation-delay: 0.46s; }
        .content-ch > *:nth-child(5) { animation-delay: 0.58s; }
      `}</style>

      <div className="relative w-44 h-44 mb-10 select-none">
        <div className="disc-ch w-44 h-44 rounded-full bg-[#1a1a1a] border-2 border-[#2a2a2a] flex items-center justify-center shadow-[0_0_40px_rgba(0,0,0,0.6)]">
          {[56, 80, 100, 124].map((d) => (
        ...
        <div className="needle-ch absolute top-[-8px] right-[-10px]" style={{ width: 6, height: 64 }}>
      ...
      <div className="content-ch space-y-4">
        ...
        <p className="text-sm text-text-secondary max-w-[36ch] mx-auto leading-relaxed">
          제 3회 오량모의국회 운영진들과의 회의를 통해<br />채팅 서비스가 종료했습니다.
        </p>
